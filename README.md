...
> [!WARNING]
> **This project is no longer actively maintained.** The TokenRhythm platform's promotional event has ended, and resale prices have skyrocketed — a key with ¥18 of credit now sells for nearly ¥3 on Xianyu (used to be under ¥2 for ¥68 of credit), so the author has given up on this project. Recommended alternatives: the author's **AMD TokenFactory Provider for Copilot** (`amdtokenfactory`, already published) or **NVIDIA NIM** (a plugin is currently in development). AMD accounts are also available from the author — feel free to reach out.
...
> [!WARNING]
> **本项目已停止维护。** 基元律动平台活动已结束，咸鱼转卖价大幅暴涨——18 元额度的 key 现在卖到了快 3 块（此前 68 元余额都卖不到 2 元），作者已放弃本项目。推荐替代：作者的 **AMD TokenFactory Provider for Copilot**（`amdtokenfactory`，已发布）或 **英伟达 NIM**（作者正在开发插件）。AMD 的账号作者也有售，欢迎联系。

...
读取 `tokenrhythm.apiKeyMode`（默认 `sticky`；非法值回退 sticky）。
...
读取 `tokenrhythm.singleKeyFallback`（默认 `switch`；非法值回退 `switch`）。
...
`GET https://tokenrhythm.studio/api/api-keys`，头 `Cookie: tr_session=<value>`，20s 超时；返回 cookie 对应账号下的全部 API Key 列表。实测仅需 `tr_session`（与 `/api/usage-summary` 同认证，无 CSRF/反爬限制）；401 抛"cookie 失效"。

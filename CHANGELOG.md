...
- **根因：自动发现模型的输出上限被错误兜底为 4096**：TokenRhythm 上架新模型后，若 models.dev 目录未收录该模型（或拉取失败，网络不稳定时经常发生），插件会把规格降级为 128K 上下文 + **4096 输出上限**。思考型模型（如 glm-5.3-flash）在复杂任务下会把 4096 token 预算全部耗在推理上，正文一个字没写就 `finish_reason=length` 结束——流正常关闭但没有任何回答文本，Copilot Chat 只能显示 "Sorry, no response was returned."。内置模型（真实上限 131072）从不受影响，这也是问题只在部分新模型上出现的原因。
- **模型规格改以 `/v1/models` 元数据为主源**：新增 `getApiModelMetadataList()`，自动发现模型的上下文长度、输出上限、视觉/推理/工具能力均优先取自平台 `/v1/models` 返回的完整元数据（平台数据比 models.dev 更准更新），models.dev 仅提供友好名称与回退规格。**两源均未知输出上限时不再发送 `max_completion_tokens`**，交由服务端默认值处理，彻底消除错误截断。
- **零正文预算耗尽检测**：流式处理结束后，若结束原因为 `length`/`max_tokens` 且累计正文为空，抛出明确错误（「模型将全部输出 token 预算耗在了思考上……请降低推理强度或关闭思考后重试」）并记录 `request.zeroAnswer` 日志，不再静默返回空流。三个协议（OpenAI/Anthropic/Responses）全覆盖。

### 修复模型选择器上下文大小显示不一致

- **「同一模型一会儿显示 1M、一会儿显示 106K」**：与空响应同根因——models.dev 拉取成功时用真实 1M 上下文，拉取失败时降级为 128K 兜底（×0.8 ratio + 4096 输出 ≈ 106K）。规格改用 `/v1/models` 主源后，两个来源的数值一致，显示不再随网络状况抖动。
- **models.dev 空缓存污染修复**：首次拉取失败不再把空目录缓存一小时，改为 60 秒退避后重试，网络恢复后立即自愈。

### 模型清单更新

- **新增 GLM-5.3 / GLM-5.3-Flash**：内置模型清单加入 `glm-5.3`（纯文本，1M 上下文 / 131K 输出，支持思考切换）与 `glm-5.3-flash`（支持文本与图像输入，其余规格相同），经 `/v1/models` 实测确认。内置模型总数 14 → 16。

### 其他修复

- **自动发现模型配置改为浅拷贝返回**：修复一个潜在 bug——同一会话内连续请求自动发现模型时，上一次请求的思考开关/推理强度等修改会泄漏到后续请求（如上一轮关闭思考后，下一轮未重新选择时思考仍保持关闭）。
